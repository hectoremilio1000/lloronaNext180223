...
function MenuDetail() {
  ...
  const filterCategory = (category) => {
    ...
    const subcategories = [
      ...new Set(filterData.map((plato) => plato.list_category)),
    ];
    // console.log(subcategories)
    setPlatillos(
      platos.filter((plato) => plato.list_category === subcategories[0])
    );
  ...
  // subcatyegories
  const subfilterData = platos.filter(
    (plato) => plato.category === viewCategory
  );
  const subcategory_platos = [
    ...new Set(subfilterData.map((plato) => plato.list_category)),
  ];

  const [platillos, setPlatillos] = useState(
    platos.filter((plato) => plato.list_category === subcategory_platos[0])
  );
  ...
  const filterSubCategory = (subcategory) => {
    const subdata = platos.filter(
      (plato) => plato.list_category === subcategory
    );
                ...
                return (
                  <div className="card-detail" key={plato.id}>
                    <span className="separator">
                      <span
                        className="content-name-plat"
                        onClick={() => showModalEditar(plato)}
                      >
                        {plato.nombre}
                      </span>
                    </span>
