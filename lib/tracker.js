/**
 * First-party tracking library.
 *
 * Captures:
 *   - Click IDs: fbclid, gclid, ttclid, msclkid
 *   - UTMs: source, medium, campaign, content, term
 *   - Referrer, landing page, user agent
 *
 * Persists:
 *   - _gs_lead_id cookie (2 years, first-party)   → lead identity across sessions
 *   - _gs_first_attr localStorage                  → first-touch UTMs / click IDs
 *   - _gs_last_attr  sessionStorage                → last-touch for current session
 *
 * Sends touchpoints to pos_tracking_api (process.env.NEXT_PUBLIC_TRACKING_API_URL).
 *
 * Usage from _app.js:
 *   import { initTracker, trackPageview } from '../lib/tracker';
 *   useEffect(() => { initTracker({ site: 'llorona' }) }, []);
...
function computeAttribution() {
  ...
  return {
    ...
    /* Expose raw tiers too for debugging / future reporting */
    _first: firstAttr,
    _last: lastAttr,
  };
}
 ...
 *
 *   buildBookingWidgetUrl('http://localhost:5174', 'llorona', 'reservaciones-llorona')
 *   → 'http://localhost:5174/?slug=llorona&type=reservaciones-llorona&lead_uid=ld_xyz&utm_source=tiktok&...'
 */
export function buildBookingWidgetUrl(widgetBaseUrl, slug, eventTypeSlug) {
 ...
 *
 *   buildCalendlyUrl('https://calendly.com/team/demo?hide_gdpr_banner=1')
 *   → 'https://calendly.com/team/demo?hide_gdpr_banner=1&utm_content=ld_xyz&utm_source=tiktok&...'
 */
export function buildCalendlyUrl(baseUrl) {
  ...
  /* Calendly honors utm_source/utm_campaign/etc and includes them in the
   * webhook payload under tracking.* — we ride that field to pass lead_uid. */
  ...
  /* Custom field picked up server-side as tracking.utm_site */
